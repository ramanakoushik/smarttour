...
class SmartTourApp {
  ...
  setupEventListeners() {
    ...
    document
      ...
      ?.addEventListener("click", this.startTour.bind(this));
    document
      .getElementById("explore-demo")
      ?.addEventListener("click", this.exploreDemo.bind(this));

    // Location search
    document
      .getElementById("search-location")
      ?.addEventListener("click", this.searchLocation.bind(this));
    document
    ...
    document
      .getElementById("generate-itinerary")
      ?.addEventListener("click", this.generateItinerary.bind(this));
    ...
    // AR/VR buttons
    document
      .getElementById("ar-preview")
      ?.addEventListener("click", this.openARPreview.bind(this));
    document
      .getElementById("vr-tour")
      ?.addEventListener("click", this.openVRTour.bind(this));

    // Analytics controls
    document
      .getElementById("time-range")
      ?.addEventListener("change", this.updateAnalytics.bind(this));
    document
  ...
  updateLanguage() {
    ...
    const currentTranslations =
      translations[this.currentLanguage] || translations.en;
  ...
  async generateItinerary() {
    ...
    try {
      ...
      const itinerary = this.createSampleItinerary(
        destination,
        startDate,
        endDate,
        travelStyle
  ...
  createSampleItinerary(destination, startDate, endDate, travelStyle) {
    ...
    for (let day = 1; day <= days; day++) {
      const dayActivities = [];
      const selectedActivities = this.selectedInterests
  ...
  setupFeedbackForm() {
    ...
    if (feedbackForm) {
      feedbackForm.addEventListener(
        "submit",
        this.handleFeedbackSubmit.bind(this)
      );
    }
    ...
    if (loadMoreReviewsBtn) {
      loadMoreReviewsBtn.addEventListener(
        "click",
        this.loadMoreReviews.bind(this)
      );
    }
  }
            ...
            <div class="review-header">
                <div class="reviewer-info">
                    ...
                    <div class="reviewer-details">
                        ...
                        <span class="review-location">Visited ${
                          feedbackData.location || "Jharkhand"
                        }</span>
                    </div>
                </div>
        ...
        const additionalReviews = [
          {
            ...
            rating: 5,
            message:
              "The wildlife sanctuary was breathtaking! Perfect for photography enthusiasts.",
            date: "1 month ago",
          },
          {
            name: "Meera Das",
            ...
            rating: 4,
            message:
              "Great cultural experience. The local guides were very knowledgeable about tribal history.",
            date: "1 month ago",
          },
        ];
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const profileSelect = document.getElementById("profile-mode-select");
  ...
  if (profileSelect) {
    profileSelect.addEventListener("change", (e) => {
      ...
      applyProfileMode(mode);
      if (mode === "admin" && analyticsSection) {
        analyticsSection.scrollIntoView({ behavior: "smooth" });
      }
    });
    applyProfileMode(profileSelect.value || "tourist");
  }
});
